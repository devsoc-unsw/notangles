...
/**
 * Gets all the data for a class
 * @param data The data array to scrape data from
 * @param rowStartIndex index to start scraping from
 */
const getClassData = async (data, rowStartIndex) => {
  ...
  for (const field of fields) {
    ...
    if (
      ...
      const enrolmentSkipAlignment = 9
      return [[], rowStartIndex + enrolmentSkipAlignment, -1]
    }
    classData[field] = data[rowStartIndex]
    rowStartIndex++
  }

  // Find out the term the class runs in.
  if (termField.charAt(0) === 'U') {
    term = 0
  } else {
    term = parseInt(termField.charAt(1))
  }

  // Get the status of the class
  const stausRegex = />([^\<]+)</
  let result = stausRegex.exec(data[rowStartIndex])
...
const scrapePage = async page => {
  // List of all courses on the page (there may be more than 1)
  const coursesData = []
  let rowStartIndex = 0
  ...
  while (rowStartIndex < data.length - 2) {
    ...
    const courseHead = await page.evaluate(() => {
      ...
      let titleRegex = /(^[A-Z]{4}[0-9]{4})(.*)/
      return titleRegex.exec(courseHeader)
    })
...
/**
 * Creates browser pages to then use to scrape the site
 * @param browser Browser in which to create new pages
 * @param batchsize Number of pages needed
 */
