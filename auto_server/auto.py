...
# times of days of classes (e.g. 9, which represents 9:00 AM) and durations (given in hours) all get multiplied this value and int-casted as the model only allows integers
TIME_MULT = 2
...
# a duration in hours that lets end time interval fill the rest of the day
SUIABLE_END_TIME_BUFFER = 26


def isComposedOfPairsOfPeriodsOnTheSameDay(periodsPerClass, periodTimes):
...
def reducePeriodInfo(period):
    ...
    # error check: if no periodsPerClass then default to 1 period with duration 1 hr
    ...
    if periodsPerClass == 1:
        return (
            ...
            [int(periodTimes[i] * DAY_MULT + TIME_MULT * periodTimes[i + 1])
    ...
    if isComposedOfPairsOfPeriodsOnTheSameDay(periodsPerClass, periodTimes) and period:
        return (
...
def sols(requestData):
    ...
    newPeriodData = [reducePeriodInfo(
        l) for l in requestData.periodInfo]  # reduces data
    ...
    normalClassIndices = [i for i in range(
    ...
    for specIndex in range(numCourses):
        ...
        specialBools = [model.NewBoolVar(f"e{i}")
        ...
        groupStartTimes = [model.NewIntVar(
            MIN_TIME, MAX_TIME, f"s{specIndex}{i}") for i in range(len(durations))]
    ...
    for i in range(1, 6):
        ...
        # extends from the start of the day to the earliest start time
        laterThanArr.append(model.NewOptionalFixedSizeIntervalVar(
            i * DAY_MULT, earliestStartTime, newBools[0], f'l{i}'))
        noLaterThanArr.append(model.NewOptionalFixedSizeIntervalVar(i * DAY_MULT + latestEndTime + minGapBetw, SUIABLE_END_TIME_BUFFER,
                              newBools[1], f'nl{i}'))  # extends from latest end time to the end of the day (i.e. the next day)
    ...
    if maxDays < 5:
        ...
        possibleDays = [model.NewIntVar(
            1, 6, f"dv{i}") for i in range(maxDays)]
    ...
    model.AddNoOverlap(classIntervals + laterThanArr +
                       noLaterThanArr + specialIntervalVars)

    # try to satisfy as many constraints as possible
    model.Maximize(cp_model.LinearExpr.WeightedSum(
        constraintBools, constraintWeights))
    ...
    if solver.StatusName(status) != "INFEASIBLE" and unsatisfied <= MAX_UNSATISFIED_CONSTRAINTS:
        solutions = [solver.Value(classStartTimes[i])
