...
def reduePeriodInfo(period):
    ...
    Args:
        period ([periodsPerClass, periodTimes[], durations[]]): _description_
    ...
    if periodsPerClass == 1:
        return (
            int(durations[0] * 2),
            [int(periodTimes[i] * 100 + 2 * periodTimes[i + 1])
...
def sols(requestData):
    ...
    for specIndex in range(numCourses):
        ...
        groupStartTimes = [model.NewIntVar(
            100, 560, f"s{specIndex}{i}") for i in range(len(durations))]
    ...
    laterThanArr = [model.NewFixedSizeIntervalVar(
        i * 100, earliestStartTime, f"l{i}") for i in range(1, 6)]
    noLaterThanArr = [model.NewFixedSizeIntervalVar(
        i * 100 + latestEndTime + minGapBetw, 16, f"l{i}") for i in range(1, 6)]
    ...
    if maxDays == 1:
        ...
        for i in range(len(classStartTimes)):
            model.AddDivisionEquality(
                classDayTimes[i], classStartTimes[i], 100)
