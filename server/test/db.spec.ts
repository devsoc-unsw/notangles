...
describe('Integration testing for user/friend db endpoints', () => {
    ...
    beforeAll(async () => {
        ...
        process.env.DATABAsSE_URL = `postgresql://${process.env.POSTGRES_USER}:${process.env.POSTGRES_PASSWORD}@localhost:${process.env.POSTGRES_PORT}/${process.env.POSTGRES_DB}?schema=public`;
    ...
    describe('Friends activity fetching tests', () => {
        const [user1, user2, user3] = mockData.users;
        beforeEach(async () => {
            for (const user of [user1, user2, user3]) {
                const res = await request(app.getHttpServer())
        ...
        test('User 1 sends req to 2, accepts req from 1 - adding friends succesfully', async () => {
            // User 1 sends friend request to 2
            let res = await request(app.getHttpServer())
                .post('/friend/request')
                .send({ senderId: user1.userID, sendeeId: user2.userID });
            ...
            // Confirm user 1 is friends with user2
            res = await request(app.getHttpServer()).get(`/friend/${user1.userID}`);
            expect(res.status).toEqual(200);
            expect(res.body.data).toHaveLength(2);
            expect(res.body.data.map((u) => u.userID)).toEqual(
                expect.arrayContaining([user2.userID]),
            );

            // user 1 is friends with user2
            // test getAllFriends - should return user2's current activity
            res = await request(app.getHttpServer()).get(`/friend/findAllFriends/${user1.userID}`);
            expect(res.status).toEqual(200);
            expect(res.body.data).toHaveLength(2);
            // friends have no current activity
            res.body.data.forEach(friend => {
                expect(friend.currentActivity).toBeNull();
            });
           
        });


        test('Should return correct current activity for a user with an ongoing class', async () => {
            const userId = mockData.users[1].userID;

            // fake the date to be 2013-10-21T13:28:06.419Z (matching the mock data)
            jest.spyOn(global, 'Date').mockImplementation(() => new Date('2013-10-21T13:28:06.419Z'));

            const res = await request(app.getHttpServer()).get(`/friend/findAllFriends/${userId}`);

            expect(res.status).toEqual(200);
            expect(res.body.data).toBeInstanceOf(Array);

            const friend = res.body.data.find(f => f.userId === mockData.users[1].userID);
            expect(friend.currentActivity).toBeTruthy();
            expect(friend.currentActivity.type).toBe('event');
            expect(friend.currentActivity.name).toBe('event1');
        });
