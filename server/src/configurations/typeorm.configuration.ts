...
export class TypeOrmConfigService implements TypeOrmOptionsFactory {
  private readonly CONFIGURATION: Readonly<TypeOrmModuleOptions> = {
    ...
    database: this.configService.get<string>('TYPEORM_DATABASE', 'notangles'),
    host: this.configService.get<string>('TYPEORM_HOST', 'localhost'),
    password: this.configService.get<string>('TYPEORM_PASSWORD', 'notangles'),
    port: this.configService.get<number>('TYPEORM_PORT', 5432),
    synchronize: this.configService.get<boolean>('TYPEORM_SYNCHRONIZE', true),
    type: 'postgres',
    username: this.configService.get<string>('TYPEORM_USERNAME', 'notangles'),
  };
