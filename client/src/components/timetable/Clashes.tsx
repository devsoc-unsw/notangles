...
import { Activity, CourseCode, ClassData, ClassPeriod, ClassTime, EventPeriod, EventTime, InInventory, InventoryPeriod } from '../../interfaces/Periods';
...
const checkClashes = (clashes: Set<ClassPeriod | EventPeriod>, card1: (ClassPeriod | EventPeriod)[], card2: (ClassPeriod | EventPeriod)[]) => {
...
export const getClashes = () => {
    ...
    if (currSelectedClasses !== null) {
        ...
        checkClashes(clashes, classPeriods, classPeriods);
    }

    if (eventPeriods !== null) {
        checkClashes(clashes, eventPeriods, eventPeriods);
    }

    if (currSelectedClasses !== null && eventPeriods !== null) {
        const classPeriods = getClassPeriods(currSelectedClasses);
        checkClashes(clashes, classPeriods, eventPeriods);
    }
...
export const sortClashesByTime = (clashDays: Record<number, (ClassPeriod | EventPeriod)[]>) => {
...
export const sortClashesByDay = (clashes: (ClassPeriod | EventPeriod)[]) => {
...
export const groupClashes = (sortedClashes: Record<number, (ClassPeriod | EventPeriod)[]>) => {
    const { days } = useContext(AppContext);
...
export const getClashInfo = (groupedClashes: Record<number, (ClassPeriod | EventPeriod)[][]>, card: ClassCard | EventPeriod) => {
    ...
    if (card.type !== 'inventory') {
        ...
        clashGroup.forEach((clash) => {
            ...
            // // If event and anything clashes --> clashColour = transparent 
            // if (card.type === 'event' ) {
            //     isEventClash = true;
            // }

            // // If anything and event clashes --> clashColour = transparent 
            // if (clash.type === 'event') {
            //     isEventClash = true;
            // }
        });
