...
export const useColorMapper = (arr: string[]): Record<string, string> => {
  const [assignedColours, setAssignedColours] = useState<Record<string, string>>({})

  useEffect(() => {
    const takenColors = new Set<string>()
    const newAssignedColours: Record<string, string> = {}
    for (const item of arr) {
      const color = (item in assignedColours)
    ...
    if(JSON.stringify(assignedColours) !== JSON.stringify(newAssignedColours)) {
