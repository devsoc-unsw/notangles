...
import { Card } from '../interfaces/Timetable';
...
export const classTranslateX = (
  ...
  if (isScheduledPeriod(card) && clashIndex !== undefined && width && cellWidth) {
    ...
    return `${String((cellWidth + 1) * card.time.day + clashIndex * (cellWidth / numClashing))}px`;
 ...
 */
export const classTranslateY = (classCard: Card, earliestStartTime: number, y?: number) => {
  let result = 0;
  ...
  if (isScheduledPeriod(classCard)) {
    ...
    const offsetRows = classCard.time.start / 60 - earliestStartTime;
  ...
  return `calc(${String(result * 100)}% + ${result.toString()}px)`;
};

export const isScheduledPeriod = (data: Card) => data.type !== 'unscheduled';

/**
 * @param card The card
 * @returns The scale factor of a card's height based on its duration relative to a standard one hour class
 */
export const getHeightFactor = (card: Card) => (isScheduledPeriod(card) ? (card.time.end - card.time.start) / 60 : 1);

/**
 * @param card The card
 * @returns The height of a class in pixels based on its duration
 */
export const getClassHeight = (card: Card) => {
